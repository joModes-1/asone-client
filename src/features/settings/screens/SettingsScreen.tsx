 ...
 *
 * Two sections are honest about doing nothing yet:
 *
 *   Synchronization       there is no offline mode in this app. The values
 *                         save and read back correctly; nothing reads them
 *                         at runtime today.
 *   Printing Preferences  there is no packing-list print or export feature
 *                         to configure yet. Same story — stored, not acted
 *                         on.
 *
 * System Alerts is the one section with real, immediate effect: each toggle
 * turns that alert kind off in `dashboard.services.needs_attention()` for
 * everyone, the instant it saves — org-wide, because there is no per-user
 * notification delivery anywhere in this system to prefer instead.
 */
  ...
  return (
    <AppShell title="Settings">
      ...
      <div className="settings-row-2col">
        ...
        <fieldset className="settings-section" disabled={!canEdit}>
          <legend className="settings-section__title">Synchronization</legend>
          <p className="settings-section__note">
            Stored for a future offline mode. Nothing reads these values today.
          </p>
          <TextField
            label="Auto-Sync Interval (minutes)"
            type="number"
            min={1}
            value={draft.auto_sync_interval_minutes}
            onChange={(event) => update('auto_sync_interval_minutes', Number(event.target.value))}
          />
          <TextField
            label="Offline Data Retention (days)"
            type="number"
            min={1}
            value={draft.offline_data_retention_days}
            onChange={(event) => update('offline_data_retention_days', Number(event.target.value))}
          />
        </fieldset>
      </div>
